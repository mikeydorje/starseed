...
function computeSeedValues() {
  ...
  return {
    ...
    rotSpeedY: 0.003 + feedback * 0.045, rotSpeedX: 0.002 + feedback * 0.022, smoothing: 0.96 - tremolo * 0.15, detail: Math.floor(8 + tremolo * 24),
...
AudioStore.load().then(data => { if (!data) { document.getElementById('audio-loader').style.display='none';document.getElementById('upload-area').style.display=''; return; } currentFileName = data.name; if (!audioContext) initAudio(0.96); audioContext.decodeAudioData(data.buffer, buf => { currentBuffer = buf; audioDuration = buf.duration; showAudioReady(); }); }).catch(() => { document.getElementById('audio-loader').style.display='none';document.getElementById('upload-area').style.display=''; });
...
function animate() {
  ...
  const td = Math.sin(dt * TP / (DRIFT_BASE * 1.38) + 2.0 + (_dp._td || 0)) * 0.18;
  particles.rotation.y = _motionT * rotSpeedY * (arc.rot || 1) * (1.0 + rd) * 0.65;
  particles.rotation.x = _motionT * rotSpeedX * 0.2 * (arc.rot || 1) * (1.0 + td) * 0.65;
...
window.SCENE = {
  ...
  rotXMult: 0.13, rotDriftScale: 0.25, tiltDriftScale: 0.18,
  rotYMult: 0.65, posDrift: { amt: 0.18, px: 2.0, py: 1.6, ys: 0.7, pz: 2.8, zs: 0.4 }, breathe: { period: 2.5, amp: 0.09 },
  storyArc,
