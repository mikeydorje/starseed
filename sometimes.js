const vertexShader = `
  ...
  varying float vGlow;
  const float VIS_INPUT_GAIN = 0.31622777; // -10 dB visual attenuation
  ...
  void main() {
    ...
    float tend = sin(uTime * 0.01 + aPhase * 2.0 + aCluster * 3.0) * uTenderness * 0.22;
    float tendY = cos(uTime * 0.008 + aPhase * 1.5) * uTenderness * 0.18;

    // Warmth: audio-reactive bloom — expansion toward neighbors
    float bloom = (0.25 + gAmp * 0.75) * uWarmth * 0.35 * sin(aOrbit * 4.0 + uTime * 0.015);

    // Closeness: clusters contract — interdependent, pulling together
    float pull = sin(uTime * 0.005 + aCluster * 5.0) * uCloseness * 0.25 * (0.25 + gAmp2 * 0.75);
    float pullY = cos(uTime * 0.006 + aOrbit * 3.0) * uCloseness * 0.18;
    ...
    gl_PointSize = aSize * (4.0 * uEnvelope + gAmp * 5.5 + uWarmth * 2.5 * uEnvelope + gAmp2 * 1.0) * (300.0 / -mvPos.z);
...
function computeSeedValues() {
  ...
  return {
    ...
    rotSpeedY: 0.003 + warmth * 0.035, rotSpeedX: 0.0015 + warmth * 0.017, smoothing: 0.97 - tenderness * 0.22, detail: Math.floor(5 + tenderness * 6),
...
function animate() {
  ...
  particles.position.x = Math.sin(dt * TP / (DRIFT_BASE * 2.3) + (_dp._px || 0)) * driftAmt;
  particles.position.y = Math.sin(dt * TP / (DRIFT_BASE * 1.9) + 1.7 + (_dp._py || 0)) * driftAmt * 0.6;
  particles.position.z = Math.sin(dt * TP / (DRIFT_BASE * 2.7) + 0.9 + (_dp._pz || 0)) * driftAmt * 0.4;
  const breathe = 1.0 + Math.sin(dt * TP / (DRIFT_BASE * 2.5) + (_dp._br || 0)) * 0.09 * (arc.rot || 1);
  particles.scale.setScalar(breathe);
  const rd = Math.sin(dt * TP / (DRIFT_BASE * 0.92) + (_dp._rd || 0)) * 0.23;
  const td = Math.sin(dt * TP / (DRIFT_BASE * 1.38) + 2.0 + (_dp._td || 0)) * 0.16;
  particles.rotation.y = _motionT * rotSpeedY * (arc.rot || 1) * (1.0 + rd) * 0.6;
  particles.rotation.x = _motionT * rotSpeedX * 0.15 * (arc.rot || 1) * (1.0 + td) * 0.6;
...
window.SCENE = {
  ...
  rotXMult: 0.09, rotDriftScale: 0.23, tiltDriftScale: 0.16,
  rotYMult: 0.6, posDrift: { amt: 0.15, px: 2.3, py: 1.9, ys: 0.6, pz: 2.7, zs: 0.4 }, breathe: { period: 2.5, amp: 0.09 },
  storyArc,
