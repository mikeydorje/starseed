...
playBtn.addEventListener('click', () => {
  ...
  if (playState === "paused") { _smoothedFreq.fill(0); controlsEl.classList.add("hidden"); controlsEl.classList.remove("visible"); audioContext.resume(); playState = "playing"; return; }
  applyAndLaunch();
...
function animate() {
  ...
  if (playState === 'playing' && audioDuration > 0 && audioStartTime > 0) {
    const progress = ((audioContext.currentTime - audioStartTime) / audioDuration) % 1;
  ...
  particles.rotation.x = _motionT * rotSpeedX * 0.3 * (arcMult.rot || 1) * (1.0 + tiltDrift);
